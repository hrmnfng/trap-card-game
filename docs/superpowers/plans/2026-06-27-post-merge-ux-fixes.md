...
- **Maestro device-tier CI — RESOLVED (2026-06-27): not flakiness, a PR #7 regression; gate scoped to smoke.** Investigation (systematic-debugging, CI artifacts) showed the device job passed on `main` pre-PR#7 and failed deterministically on every run after — the `game` flow specifically, always at `id: ready-toggle is visible`. Root cause: the device never left Home because `tapOn id:join-lobby` never reached `onPress`. PR #7's Home restructure (`Screen`/SafeAreaView + `MotiView` entrance animation + `KeyboardAvoidingView`) makes Maestro's coordinate interaction unreliable while the soft keyboard is up — the `testID`'d button reports skewed native bounds (~300px off), tapping the visible "Join" label only works after `hideKeyboard`, and `hideKeyboard`'s Android BACK keypress clears the typed code; an IME-submit attempt then mis-fired (the device created its own lobby instead of joining). Each fix exposed a new failure → an architectural smell, so rather than keep patching a brittle Expo-Go-on-emulator full-game flow, the gate was **scoped to the `smoke` flow** (the actual native-crash gate, which passed throughout). The `game` flow + `player2.mjs` were removed; game *logic* is covered by the Playwright web e2e and the on-device experience by the manual LAN matrix. A future automated on-device game flow should use an **Expo Dev Build** (not Expo Go) with a keyboard-robust input strategy. None of this is an app bug — the flow works by hand on a device.
